...
ScheduleTool.dayJob(2, async () => {
  ...
  let onlyRecord = await DB.DurationRecord.findAll({
    attributes: [
      [DB.sequelize.fn("DISTINCT", DB.sequelize.col("account_id")), "accountId"],
    ],
...
server.listen(8081, () => {
  console.log("服务启动在：https://backend-course.xdproject.top");
});
